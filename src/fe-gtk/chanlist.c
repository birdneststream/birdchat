...
chanlist_filereq_done (server *serv, char *file)
{
	...
	g_snprintf (buf, sizeof buf, "BirdChat Channel List: %s - %s\n",
...
chanlist_button_cb (GtkTreeView *tree, GdkEventButton *event, server *serv)
{
	...
	mg_create_icon_item (_("_Join Channel"), "go-next", menu,
								chanlist_join, serv);
...
chanlist_opengui (server *serv, int do_refresh)
{
	...
	wid = gtkutil_button (NULL, "go-next", 0, chanlist_join, serv,
						 _("_Join Channel"));

...
/* GTK3: Background drawing now handled by CSS/Cairo */
/* #define xtext_draw_bg(xt,x,y,w,h) gdk_draw_rectangle(xt->draw_buf, xt->bgc, 1, x, y, w, h); */
#define xtext_draw_bg(xt,x,y,w,h) /* TODO: Implement with Cairo */
...
{
	/* GTK3: Use pango_cairo_show_layout_line for reliable text rendering */
	cairo_move_to (cr, x, y);
	pango_cairo_show_layout_line (cr, line);
}

static void
xtext_set_fg (GtkXText *xtext, cairo_t *cr, int index)
{
	if (cr)
		gdk_cairo_set_source_rgba (cr, &xtext->palette[index]);
}

static void
xtext_set_bg (GtkXText *xtext, cairo_t *cr, int index)
{
	if (cr)
		gdk_cairo_set_source_rgba (cr, &xtext->palette[index]);
}
...
static void
backend_draw_text_emph (GtkXText *xtext, int dofill, cairo_t *cr, int x, int y,
						 char *str, int len, int str_width, int emphasis)
{
	...
	/* Ensure valid UTF-8 before passing to Pango */
	if (g_utf8_validate (str, len, NULL)) {
		pango_layout_set_text (xtext->layout, str, len);
	} else {
		/* Replace invalid UTF-8 with replacement characters */
		gchar *valid_str = g_utf8_make_valid (str, len);
		pango_layout_set_text (xtext->layout, valid_str, -1);
		g_free (valid_str);
	}

	if (dofill && cr)
	{
		/* Set background color and fill background rectangle */
		xtext_set_bg (xtext, cr, xtext->col_back);
	...
	if (cr) {
		...
		xtext_set_fg (xtext, cr, xtext->col_fore);
		cairo_move_to (cr, x, y);
		xtext_draw_layout_line (cr, xtext, x, y, line);
	}
}
...
gtk_xtext_adjustment_set (xtext_buffer *buf, int fire_signal)
{
	...
	if (buf->xtext->buffer == buf)
	{
		...
		/* gtk_adjustment_value_changed not needed in GTK3 - signals fire automatically */
		/* if (fire_signal)
...
gtk_xtext_new (GdkRGBA palette[], int separator)
{
	...
	/* GTK3: Double buffering is automatic and always enabled */
...
gtk_xtext_realize (GtkWidget * widget)
{
	...
	/* GTK3: GdkGCValues and GdkColor no longer needed */
	/* GTK3: GdkColormap no longer needed */
	...
	/* GTK3: No need for explicit colormap setup */
	...
	/* GTK3: All GdkGC drawing replaced with Cairo drawing in draw event */
	/* Draw buffer is now managed by GTK3 automatically via Cairo */

	/* GTK3: Background pixmap handling replaced with CSS styling */
	if (xtext->pixmap)
	{
		/* Background pixmap will be handled in Cairo draw event */
	...
	/* GTK3: Background styling handled automatically */
...
static void
gtk_xtext_get_preferred_width (GtkWidget *widget, gint *minimum_width, gint *natural_width)
{
	*minimum_width = *natural_width = 200;
}

static void
gtk_xtext_get_preferred_height (GtkWidget *widget, gint *minimum_height, gint *natural_height)
{
	*minimum_height = *natural_height = 90;
}
...
{
	int x, height;
	/* GTK3: GdkGC replaced with cairo_t */
	cairo_t *light, *dark;
	...
	if (xtext->separator && xtext->buffer->indent)
	{
		...
		if (xtext->thinline)
		{
			/* GTK3: Line drawing with Cairo - TODO */
		...
		{
			/* GTK3: Separator line drawing with Cairo - TODO */
			/* if (xtext->moving_separator)
			{
...
gtk_xtext_draw_marker (GtkXText * xtext, textentry * ent, int y)
{
	...
	/* GTK3: Marker line drawing with Cairo - TODO */
...
/*	backend_set_clip (xtext, area);*/
	...
	if (y && y < widget_alloc.height && !ent_end->next)
	{
		GdkRectangle rect;

		rect.x = 0;
		rect.y = y;
		rect.width = widget_alloc.width;
		rect.height = widget_alloc.height - y;
...
gtk_xtext_draw (GtkWidget * widget, cairo_t *cr)
{
	...
	/* Store Cairo context for use during rendering */
	xtext->fgc = cr;
	xtext->bgc = cr;
	gtk_xtext_paint (widget, &area);
	xtext->fgc = NULL;  /* Clear after use */
	xtext->bgc = NULL;
...
gtk_xtext_scrolldown_timeout (GtkXText * xtext)
{
	...
	gdk_window_get_device_position (gtk_widget_get_window (GTK_WIDGET (xtext)), gdk_device_manager_get_client_pointer (gdk_display_get_device_manager (gdk_window_get_display (gtk_widget_get_window (GTK_WIDGET (xtext))))), NULL, &p_y, NULL);
...
gtk_xtext_class_init (GtkXTextClass * class)
{
	...
	object_class->finalize = gtk_xtext_destroy;
...
gtk_xtext_render_flush (GtkXText * xtext, int x, int y, unsigned char *str,
								int len, cairo_t *gc, int *emphasis)
{
	int str_width, dofill;
	/* GTK3: GdkDrawable replaced with cairo surface */
	...
	/* GTK3: gdk_pixmap_new deprecated - using Cairo surfaces */
	/* pix = gdk_pixmap_new (xtext->draw_buf, str_width, xtext->fontsize, xtext->depth); */
	pix = NULL; /* Temporarily disabled */
	if (pix)
	{
		dest_x = x;
		dest_y = y - xtext->font->ascent;

		/* GTK3: GdkGC deprecated */
	...
	/* Set foreground color before drawing text */
	if (gc)
	...
	if (pix)
	{
		...
		if (gdk_rectangle_intersect (&clip, &dest, &dest))
			...
			/* GTK3: Pixmap drawing with Cairo - TODO */
	...
	if (xtext->strikethrough)
	{
		...
		/* GTK3: Strikethrough line with Cairo - TODO */
...
gtk_xtext_set_palette (GtkXText * xtext, GdkRGBA palette[])
{
	...
	if (gtk_widget_get_realized (GTK_WIDGET(xtext)))
	{
		...
		/* GTK3: Color setting handled in draw callback */
...
/* GTK3: Temporarily disabled function due to GdkGC migration */
/*
void
gtk_xtext_set_background (GtkXText * xtext, GdkPixbuf * pixmap)
{
	...
	} else if (gtk_widget_get_realized (GTK_WIDGET(xtext)))
	{
		...
		/* GTK3: GdkGC is deprecated - using cairo_t instead */
...
gtk_xtext_set_background (GtkXText * xtext, GdkPixbuf * pixmap)
{
	/* GTK3: Simplified implementation without GdkGC */
	if (xtext->pixmap) {
		g_object_unref (xtext->pixmap);
		xtext->pixmap = NULL;
	}
...
/* Calculate number of actual lines (with wraps), to set gtk_adjustment_get_lower (adj). *
...
gtk_xtext_render_page (GtkXText * xtext)
{
	...
	width = gtk_widget_get_allocated_width (GTK_WIDGET (xtext));
	height = gtk_widget_get_allocated_height (GTK_WIDGET (xtext));
	...
	if (!xtext->pixmap && abs (overlap) < height)
	{
		...
		/* GTK3: Scrolling now handled differently - invalidate affected area */
		if (overlap < 1)	/* DOWN */
		{
			int remainder = ((height - xtext->font->descent) % xtext->fontsize) +
							xtext->font->descent;
